...
export default function Social({ className = "", icons = {} }) {
  const W = 53;
  const H = 12;
  ...
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      {items.map((it) => {
        ...
        // Si NO encuentra imagen, igual muestra texto para no “desaparecer”
        if (!iconUrl) {
          return (
            <Link
              key={it.key}
        ...
        return (
          ...
          >
            <Image src={iconUrl} alt={it.label} width={W} height={H} priority />
          </Link>
        );
      })}
    </div>
